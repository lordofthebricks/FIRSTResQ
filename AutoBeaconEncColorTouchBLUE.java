 ...
 */
public class AutoBeaconEncColorTouchBLUE extends LinearOpMode {
    DcMotor motorRight;
    DcMotor motorLeft;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (colorSensor.blue() >= 1) {     //Check to see if the color is BLUE
            ...
            arm.setPosition(0.9);       //Assuming we are RED, we want to push the RIGHT button
            motorLeft.setPower(0.2);
            motorRight.setPower(0.2);
            // sleep(100);

        } else if (colorSensor.red() >= 1) {
            arm.setPosition(0.1);       //Find the LEFT button to push
            motorLeft.setPower(0.2);
            motorRight.setPower(0.2);
            // sleep(100);
        }
...
*/
        //Forward to the ramp
        ROTATIONS = (toRamp + 30) / CIRCUMFERENCE;
        COUNTS = ENCODER_CPR * ROTATIONS * GEAR_RATIO;
        target = motorLeft.getCurrentPosition() + COUNTS;
